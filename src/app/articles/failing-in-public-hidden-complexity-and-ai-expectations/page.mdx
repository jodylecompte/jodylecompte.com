...
export const article = {
                                                                                                    ...
                                                                                                    description: 'AI makes hard things look easy, and that is a trap. This post is about the moment the magic wears off, when a "working prototype" meets real data, edge cases pile up, and the gap between "it runs" and "it works" turns out to be a lot wider than expected.',
                                                                                                    tracks: [
...
author field was hallucinated. The title was truncated. The structure was close enough to look
plausible but incorrect in all the ways that mattered.

All three failures point to the same issue. The heuristic approach broke because it lacked capability;
it simply could not infer semantic structure. The LLM approach broke for the opposite reason: it had the
capability, but no controllable guarantee that the output would conform exactly to the structure
required. I traded a weakness in intelligence and rigidity for an intelligence with no rigidity or
guardrails in its output.

## The Fix: Grammar-Constrained Decoding

The instinct when you see output like that is to fix the prompt. Add a rule*do not emit duplicate keys*.
Add another: _footnotes go on the paragraph block, not the top level_. That feels like iteration. It's actually
deferring understanding. I kept operating under the assumption that the AI either did not understand me, or was just
